...
docker run --name ${GERRIT_NAME} --link ${PG_GERRIT_NAME}:db -p 8080:8080 -p 29418:29418 -v ${LOCAL_VOLUME}:/var/gerrit/review_site -e WEBURL=${GERRIT_WEBURL} -e HTTPD_LISTENURL=${HTTPD_LISTENURL} -e DATABASE_TYPE=postgresql -e AUTH_TYPE=LDAP -e LDAP_SERVER=${LDAP_SERVER} -e LDAP_ACCOUNTBASE=${LDAP_ACCOUNTBASE} -d ${GERRIT_IMAGE_NAME}
